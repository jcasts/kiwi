...
* Alternate to OPTIONS for meta information
* Refactor App serializers and mime_types to a single place.
* Figure out when and how to render HTML

= TODO

* Should Resource be a Module that gets included in Models?
  - API methods are on the class
  - Difficult to hold an App instance there

* Should HTML support be optional, by default, built-in?
  require 'kiwi/html'
* Support for templating engines (erb)
    class MyApp < Kiwi::App
      serialize :html do |data|
        erb(data, view, layout)
      end

      serialize_default :html do |data|
        erb(data, view, layout)
      end
    end
...
* Kiwi commands:
  $ kiwi new <app_name>
  $ kiwi resource <resource_name> <method-names>
  $ kiwi test [sub-folder]
  $ kiwi server
  $ kiwi console
  $ kiwi bundle # Tar-up with gems for easy deploy
  $ kiwi freeze # Creates an apidoc to verify api beaking changes

* Set default media type/format.
...
* Support streaming
    def list
      # item must be validated by view
      backend.on_data{|item| stream item }
    end
...
* Serializer should be omitable, choose a good default.
  * Always supports json (comes mostly free out of the box)
